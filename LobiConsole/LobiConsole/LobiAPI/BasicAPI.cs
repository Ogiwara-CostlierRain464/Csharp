...
namespace LobiAPI
{
    public class BasicAPI
    {
        ...
        public bool Login(string mail, string password)//ここむずくね？
        {
            Console.WriteLine("ログイン開始");
            GetHeader header1 = new GetHeader()
                .setHost("lobi.co")//Hostタブにあたる
                .setConnection(true)
                .setAccept("text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8")//Client.Accept
                .setUserAgent("Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36")//Client.UserAgent
                .setAcceptLanguage("ja,en-US;q=0.8,en;q=0.6");//Client.Accept-Language

            string source = this.NetworkAPI.get("https://lobi.co/signin", header1);//URLタブ	
            string csrf_token = Pattern.get_string(source, Pattern.csrf_token, "\"");

            string post_data = string.Format("csrf_token={0}&email={1}&password={2}", csrf_token, mail, password);//TextView
        ...
        {
            PostHeader header = new PostHeader()
                .setHost("web.lobi.co")//固定
                .setConnection(true)//固定
                .setAccept("application/json, text/plain, */*")//固定…?
                .setOrigin("https://web.lobi.co")//固定
                .setUserAgent("Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36")//固定
                .setAcceptLanguage("ja,en-US;q=0.8,en;q=0.6");//固定!
